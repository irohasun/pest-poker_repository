...
6. **初期の親（出題者）をランダムに決定**
   - ゲーム開始時に、全プレイヤーの中からランダムに1人を選ぶ
   - 選ばれたプレイヤーが最初のターンの出題者となる
   - 公平性を保つため、毎回異なるプレイヤーが選ばれる可能性がある
...
**重要：順序変更**
出題者は**まず回答者を選んでから**、カードを選ぶ順序に変更されました。
これにより、誰に渡すかを先に決めてから、どのカードを渡すかを選択できます。
...
**目的**: ゲーム開始時に全員が順番に手札を確認し、1枚を場に出す

**重要ルール**: 
- **各プレイヤーは手札確認時に、必ず1枚を選択して場に出す**
- 選択したカードは手札から削除され、公開カードとして場に置かれる
- これにより、ゲーム開始時点で各プレイヤーに公開カードが1枚ずつ存在する
...
3. 選択したカードが強調表示される
4. 「決定 - 場に出す」ボタンをタップ
5. 選択したカードが手札から削除され、公開カードとして場に置かれる
6. 次のプレイヤーへ
...
**実装上の注意**:
- カード選択は必須（選択しないと「決定」ボタンは無効）
- 選択したカードは手札から削除し、`openCards`に追加
- 手札枚数（`handCount`）も1枚減らす
- 全プレイヤーが1枚ずつ場に出すと、ゲームメイン画面へ遷移
...
**自動遷移機能**:
- **出題者になったら自動的に相手選択画面に遷移**
- 「誰に渡すか」ボタンは削除され、ボタンを押す必要なし
- ゲームメイン画面に到達した時点で、出題者の場合は即座に相手選択画面へ遷移
...
#### 3.3.5 出題者：相手選択画面（最初に表示）

**目的**: カードを渡す相手を選ぶ（最初のステップ）

**重要**: この画面は**カード選択の前に表示**されます。
出題者はまず誰に渡すかを決めてから、どのカードを渡すかを選択します。
...
#### 3.3.6 出題者：カード選択画面（相手選択の後）

**目的**: 手札から押し付けるカードを選ぶ（相手選択の後）

**重要**: この画面は**相手選択の後に表示**されます。
出題者は相手を選択した後、その相手に渡すカードを選びます。
...
| 2025-01-XX | 1.10 | 初期手札確認時に1枚選択して場に出すルールを追加（全プレイヤー必須） | - |
| 2025-01-XX | 1.11 | 出題者のフロー順序を変更：まず回答者を選んでからカードを選ぶ順序に変更 | - |
| 2025-01-XX | 1.12 | ゲームメイン画面の「誰に渡すか」ボタンを削除し、出題者になったら自動的に相手選択画面に遷移するように変更 | - |
| 2025-01-XX | 1.13 | 最初の出題者をランダムに選択するように変更 | - |
